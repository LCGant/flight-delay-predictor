...
from features import (
    ...
    add_compact_weather,
    add_sched_block_features,
    add_history_origem_hora,
    add_weather_coverage_flags,
    add_distance_km,
...
def read_vra_month(path: Path) -> pd.DataFrame:
    ...
    if "situacao" in df.columns:
        df["situacao"] = (
            df["situacao"].astype(str).str.upper()
...
def unify_vra_all():
    ...
    cols_model = [
        "atraso15","delay_arrival_min","delay_departure_min",
        "icao_empresa","origem_icao","destino_icao","rota",
        "partida_prevista","chegada_prevista","partida_real","chegada_real",
        "situacao","justificativa","numero_voo","di","tipo_linha",
    ]
...
def _parse_inmet_text(raw_text: str) -> pd.DataFrame:
    ...
    hora = (
        df[col_hora].astype(str).str.replace("UTC", "", case=False, regex=False)
    ...
    out["lat"] = meta["lat"]; out["lon"] = meta["lon"]
    out["station_name"] = meta["estacao"]; out["uf"] = meta["uf"]; out["regiao"] = meta["regiao"]

    def maybe(col_like, new):
        c = next((c for c in df.columns if col_like(c)), None)
        if c: out[new] = pd.to_numeric(df[c], errors="coerce")

    maybe(lambda c: "precip" in c, "precip_mm")
    maybe(lambda c: "temperatura" in c and "hora" in c, "temp_c")
    maybe(lambda c: "umidade" in c and "rel" in c, "rh")
    maybe(lambda c: "vento" in c and "velocidade" in c, "wind_ms")
    maybe(lambda c: "rajada" in c, "gust_ms")
    return out[out["wx_dt"].notna()].copy()
...
def _load_airports_latlon() -> pd.DataFrame:
    ...
    if not cand:
        raise SystemExit("Arquivo de aeródromos não encontrado em data/raw/.")
    ...
    def _norm(s): return normalize_col(s)
    ...
    buf = StringIO("\n".join(lines))
    df = pd.read_csv(
        buf, sep=";", header=header_idx, engine="python", decimal=",",
        na_values=["", "NA", "nan", "///", "////", "-", " "],
    )
    df.columns = [normalize_col(c) for c in df.columns]
    icao_cols = [c for c in df.columns if ("oaci" in c or "icao" in c)]
    if not icao_cols:
        raise SystemExit("Não encontrei coluna OACI/ICAO no arquivo ANAC.")
    ...
    if not lat_candidates or not lon_candidates:
        raise SystemExit("Não encontrei latitude/longitude no arquivo de aeródromos.")
    ...
    def _parse_latlon(s):
        ...
        t = str(s).strip().replace("_", " ").replace("º", "°").replace("o", "°").replace(",", ".")
        hemi_m = hemi_re.search(t)
        hemi = hemi_m.group(1).upper() if hemi_m else None
        nums = [float(x.group(0)) for x in nums_re.finditer(t)]
        if not nums: return np.nan
        if len(nums) == 1:
            val = float(nums[0])
        else:
            deg, minutes, seconds = float(nums[0]), (nums[1] if len(nums) >= 2 else 0.0), (nums[2] if len(nums) >= 3 else 0.0)
    ...
    out = (
        pd.DataFrame({"icao": df[col_icao].astype(str).str.upper().str.strip(),
                      "lat": pd.to_numeric(lat, errors="coerce"),
                      "lon": pd.to_numeric(lon, errors="coerce"),})
        .dropna(subset=["icao", "lat", "lon"]).drop_duplicates(subset=["icao"])
    )
    if out.empty:
        raise SystemExit("Não consegui extrair ICAO+lat/lon válidos (tudo NaN).")
    return out
...
def ingest_weather():
    ...
    frames = []
    list_csv = list(wz_dir.rglob("*.csv")) + list(wz_dir.glob("*.csv")) + \
               list(wz_dir.rglob("*.CSV")) + list(wz_dir.glob("*.CSV"))
    for path in sorted(list_csv):
        try:
    ...
    wx = pd.concat(frames, ignore_index=True).dropna(subset=["wx_dt", "lat", "lon"]).copy()
    ...
    print(f"  Proporção sem ICAO (dist > 60km): {wx['icao'].isna().mean():.2%}")
...
def _merge_asof_by_airport(flights: pd.DataFrame, wx: pd.DataFrame, ap_col: str, time_col: str, prefix: str):
    ...
    rename = {c: f"{prefix}{c}" for c in ["precip_mm", "wind_ms", "gust_ms", "temp_c", "rh"] if c in out.columns}
...
def join_weather_to_dataset(include_destination=False):
    ...
    if not base_p.exists():
        raise SystemExit("Execute primeiro: python src/etl.py unify_all")
...
def make_trainset():
    ...
    base = add_compact_weather(base, mode="adaptive", include_destination=True)
    base = add_weather_coverage_flags(base)
    base = add_sched_block_features(base)
    base = add_history_origem_hora(base)
    base = add_congestion_features(base)
    try:
        base = add_distance_km(base)
    except Exception as e:
        print("Aviso: dist_km indisponível:", e)
    ...
    feat_cols = [
        "icao_empresa","origem_icao","destino_icao","rota",
        "mes","dia_semana","hora_partida_prevista","hora_bloco_30","hora_sin","hora_cos",
        "periodo_dia_id","is_weekend","is_feriado","is_vespera_feriado","em_ferias",
        "hist_atraso_rota_30","hist_atraso_empresa_50","hist_atraso_num_voo_10",
        "hist_vol_rota_30","hist_std_rota_30","hist_voo_prev_atrasou","hist_atraso_origem_hora_30",
        "load_origem_15","airport_size_id","congestion_ratio","congestion_bucket",
    ...
    print("✔ Trainset salvo em data/processed/train_ready.parquet (com clima compacto e features operacionais)")
...
SAFE_NUMERICAL = [
    ...
    "hist_atraso_rota_30","hist_atraso_empresa_50","hist_atraso_num_voo_10",
    "hist_vol_rota_30","hist_std_rota_30","hist_voo_prev_atrasou","hist_atraso_origem_hora_30",
    "load_origem_15","airport_size_id","congestion_ratio","congestion_bucket","atrasos_mesmo_aeroporto_1h_hist",
    "chuva_id","tempo_ruim_id","chuva_arr_id","tempo_ruim_arr_id",
    "wx_dep_cov","wx_dep_all_missing","wx_arr_cov","wx_arr_all_missing",
    "sched_block_min","sched_block_bucket",
    "dist_km",
...
def _make_hist_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"origem_icao", "destino_icao"} <= set(work.columns):
        rota = (work["origem_icao"].astype(str).str.upper().str.strip() + ">" + work["destino_icao"].astype(str).str.upper().str.strip())
    else:
    ...
    work["__rota_key"] = rota; work["__cia_key"] = cia; work["__nvoo_key"] = num_voo
    ...
    for c in ["hist_atraso_rota_30", "hist_atraso_empresa_50", "hist_atraso_num_voo_10", "hist_std_rota_30"]:
        if c in work.columns: work[c] = work[c].astype("float")
    mean_global = float(y.mean()) if len(work) else 0.0
    for c in ["hist_atraso_rota_30", "hist_atraso_empresa_50", "hist_atraso_num_voo_10"]:
        if c in work.columns: work[c] = work[c].fillna(mean_global)
    if "hist_vol_rota_30" in work.columns: work["hist_vol_rota_30"] = work["hist_vol_rota_30"].fillna(0.0)
    if "hist_std_rota_30" in work.columns: work["hist_std_rota_30"] = work["hist_std_rota_30"].fillna(0.0)
    return work
...
def add_historical_features():
    ...
    out = add_compact_weather(out, mode="adaptive", include_destination=True)
    out = add_weather_coverage_flags(out)
    out = add_sched_block_features(out)
    out = add_history_origem_hora(out)
    ...
    print("✔ Features históricas salvas em data/processed/dataset_modelagem_hist.parquet")
...
def make_temporal_splits():
    ...
    if not p.exists():
        raise SystemExit("Execute antes: unify_all (e weather/features opcional).")
    df_raw = pd.read_parquet(p)
    base = df_raw[df_raw["situacao"].str.contains("REALIZADO", na=False)].copy() if "situacao" in df_raw.columns else df_raw.copy()
    if TARGET_COL not in base.columns:
...
def main():
    ...
    parser.add_argument(
        "cmd",
        choices=["unify_all","weather","trainset","features","splits","all"],
        help="comando"
    )
    args = parser.parse_args()
    if args.cmd == "unify_all":
        unify_vra_all()
